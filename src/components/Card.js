...
const Card = ({card,isEnlarged,setEnlargedCardId}) => {
    ...
    return ( 
        <div  
            ...
            className={`card ${isEnlarged ? 'enlarge' : ''}`}
            tabIndex={0}
            onKeyDown={handleKeyPress}
            >
            {
                isEnlarged && <div className="exit">
                    <FaTimes onClick={()=>{setEnlargedCardId(null); videoRef.current.pause(); }}/>
                </div>
            }
            ...
            <div className="buttons">
                <button className={`${isTextActive ? 'clicked':''}`} onClick={()=>setIsTextActive(!isTextActive)}>Hide Text</button>
                <button className={`${isVideoActive ? 'clicked':''}`} onClick={()=>setIsVideoActive(!isVideoActive)}>Hide Video</button>
